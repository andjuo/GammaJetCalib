...
#else
#  include "../interface/testRespCorrDiJetsTree.h"
#endif
...

{
...
#else
  TString output = "QCD_test.root";
  if (withCuts) output.ReplaceAll(".root","-withCuts.root");
  TH1D* h_corr_ = new TH1D("h_corr","h_corr",100,0.,100.);
  for (int ibin=1; ibin<=h_corr_->GetNbinsX(); ++ibin) {
    h_corr_->SetBinContent(ibin, 1.);
    h_corr_->SetBinError  (ibin, 0.);
  }
#endif
  ...
  for(int iEvent=0; iEvent<nEvents; iEvent++){
    ...
    if (withCuts) {
      ...
      float ppfjet_Et = ppfjet_E_/cosh(ppfjet_eta_);
      float minSumJetEt_ = 40.0;//40.0;
      float minJetEt_ = 20.0;//20.0;
      float maxThirdJetEt_ = 15.0;//15.0;
      float maxDeltaEta_ = 0.5;//0.5;
      if(tpfjet_Et + ppfjet_Et < minSumJetEt_) failSel |= 0x1;
      if(tpfjet_Et < minJetEt_ || ppfjet_Et < minJetEt_) failSel |= 0x2;
      if(sqrt(pf_thirdjet_px_*pf_thirdjet_px_ + pf_thirdjet_py_*pf_thirdjet_py_) > maxThirdJetEt_) failSel |= 0x4;
      if(pf_dijet_deta_ > maxDeltaEta_) failSel |= 0x8;
    
      //std::cout << "iEvent=" << iEvent << " failSel=" << failSel << "\n";
      if(failSel) {
